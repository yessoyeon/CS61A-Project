...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        ...
        paired.append([find_closest(location, centroids), restaurant])
...
def find_predictor(user, restaurants, feature_fn):
    ...
    # BEGIN Question 7
    s_xx = [(x - mean(xs))**2 for x in xs]
    s_yy = [(y - mean(ys))**2 for y in ys]
    s_xy = [(x - mean(xs)) * (y - mean(ys)) for x, y in zip(xs, ys)]
    b = sum(s_xy) / sum(s_xx)
    a = mean(ys) - b * mean(xs)
    r_squared = sum(s_xy)** 2 /(sum(s_xx) * sum(s_yy))
    # END Question 7
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for feature in feature_fns:
        pred, r  = find_predictor(user, reviewed, feature)
        dic[pred] = r
...
def search(query, restaurants):
    ...
    """
    # BEGIN Question 10
    return [place for place in restaurants if query in restaurant_categories(place)]
    # END Question 10
